...
const CASES = [
  ...
  { name: 'ES (AdOpt): "Aceptar" em contêiner separado', click: true, html: `<div style="position:fixed;bottom:0;left:0;right:0;background:#222;color:#fff;padding:16px"><div><small>Nuestro sitio web utiliza cookies para mejorar la navegación.</small> <a href="#" onclick="${BAD}">Política de privacidad</a> <a href="#" onclick="${BAD}">Optar por no</a></div><div><button onclick="${ACC}">Aceptar</button></div></div>` },
  { name: 'implícito: só link de política (americanas) → NÃO clicar', click: false, html: `<div style="position:fixed;bottom:0;left:0;right:0;background:#222;color:#fff;padding:16px"><strong>Cookies:</strong> usamos cookies para personalizar. Ao continuar navegando você concorda com o nosso <a href="#" onclick="${BAD}">Aviso de Privacidade</a>.</div>` },
  { name: 'NEG: overlay genérico SEM texto de cookie ("Aceitar" solto)', click: false, html: `<div style="position:fixed;bottom:0;left:0;right:0;background:#222;color:#fff;padding:16px"><p>Receba nossa newsletter!</p><button onclick="${BAD}">Aceitar</button></div>` },
];
